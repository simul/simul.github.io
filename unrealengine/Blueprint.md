...
### Directional Lights
To set the direction and colour of a Directional Light, find the `UpdateLight` function from the trueSKY function category, and connect a reference to the DirectionalLight to the input. Make sure that the "Exec" input is hit once per frame by a tick event. If you're driving your scene lighting through a DirectionalLightComponent instead of a directional light actor, you can alternatively use `UpdateLightComponent`. 

![alt text](http://docs.simul.co/unrealengine/images/UpdateLight.png "Blueprint usage of the UpdateLight function.")
...
![alt text](http://docs.simul.co/unrealengine/images/SettingTime.png "Adding time advancement logic to the blueprint Event Tick.")
