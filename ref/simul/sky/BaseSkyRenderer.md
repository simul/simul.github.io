...
See ::clouds::BaseWeatherRenderer::Render BaseWeatherRenderer::Render ::clouds::BaseWeatherRenderer::RenderMixedResolution RenderMixedResolution\endlink.

The rendering of the sky is no longer handled by BaseSkyRenderer or its derived classes because the
::sky::BaseAtmosphericsRenderer atmospherics renderer does this in a single pass including both sky and atmospherics.
...
The table size is given by NumElevations, NumDistances and SetNumAltitudes() of ::sky::SkyKeyframer SkyKeyframer\endlink.
...
::clouds::BaseWeatherRenderer::PreRenderUpdate() BaseWeatherRenderer::PreRenderUpdate calls ::sky::BaseSkyRenderer::EnsureTexturesAreUpToDate EnsureTexturesAreUpToDate\endlink, which fills in the fade textures
...
(see ::clouds::BaseWeatherRenderer::RenderMixedResolution\endlink).
...
All this is handled automatically by the SkyKeyframer, which in turn can use a ::sky::BaseGpuSkyGenerator GpuSkyGenerator\endlink.
...
This is called by RenderPlanets to render a planet with texture **tex**, angular radius **rad** radians, in direction **dir** (x,y,z), with colour **colr**.
If **do_lighting** is true, the planet will be directionally-lit - e.g. moon phases.
This is called by RenderPlanets to render a planet with texture **tex**, angular radius **rad** radians, in direction **dir** (x,y,z), with colour **colr**.
If **do_lighting** is true, the planet will be directionally-lit - e.g. moon phases.
...
The texture **tex** is an API-specific texture identifier, e.g. a GLuint, or LPDIRECT3D9TEXTURE.
The angular radius will be **rad**, in radians. If **do_lighting** is true, the planet should be directionally lit by the sun
to produce phases.
Create a planet, identified by the **index**, which can be any unique number.
The texture **tex** is an API-specific texture identifier, e.g. a GLuint, or LPDIRECT3D9TEXTURE.
The angular radius will be **rad**, in radians. If **do_lighting** is true, the planet should be directionally lit by the sun
...
Set the texture for planet **index** to **tex**, an API-specific texture identifier, e.g. a GLuint, a DX9 LPDIRECT3D9TEXTURE, or DX11 ShaderResourceView.
Set the texture for planet **index** to **tex**, an API-specific texture identifier, e.g. a GLuint, a DX9 LPDIRECT3D9TEXTURE, or DX11 ShaderResourceView.
...
Inform the derived class that the loss and inscatter textures 
**texture_index**, either 0, 1 or 2, at altitude
index 
**alt_index**, (not used if not using multiple altitudes) should fill a block of texels
starting at **x**, **y**, **z**, and **w** wide, **l** long, **d** deep, with the values in the array **uint32_array**,
with the values in the arrays 
**loss_float4_array** and 
**inscatter_float4_array**, each of which contains
four times 
**num_texels** floats. The first number is the value for 
