...
| simul::sky::float4 | [GetColourLossFactor](#GetColourLossFactor)(simul::sky::float4 view_pos_km, simul::sky::float4 pos_km, simul::sky::HazeStruct hazeStruct, bool earth_test, int steps) |
| simul::sky::float4 | [GetDirectionToSun](#GetDirectionToSun)(double epoch_time, float latitude, float longitude) |
...
| vec3 | [GetLocalMoonIrradianceFactor](#GetLocalMoonIrradianceFactor)(float altitude_km, simul::sky::float4 dir_to_moon, simul::sky::float4 dir_to_sun, simul::sky::HazeStruct hazeStruct) |
| vec3 | [GetLocalSunIrradiance](#GetLocalSunIrradiance)(float altitude_km, vec3 sun_irradiance, simul::sky::float4 dir_to_sun, simul::sky::HazeStruct hazeStruct) |
...
| float | [GetPlanetRadius](#GetPlanetRadius)() |
...
### <a name="GetColourLossFactor"/>simul::sky::float4 GetColourLossFactor(simul::sky::float4 view_pos_km, simul::sky::float4 pos_km, simul::sky::HazeStruct hazeStruct, bool earth_test, int steps)
When an object has initial colour **c**, multiply each member of **c** by the corresponding member of
the loss factor to get the colour as seen by the viewer.
When an object has initial colour **c**, multiply each member of **c** by the corresponding member of
the loss factor to get the colour as seen by the viewer.

### <a name="GetDirectionToSun"/>simul::sky::float4 GetDirectionToSun(double epoch_time, float latitude, float longitude)
...
this function returns how far a ray cast at angle **Elevation** above the horizon would travel
...
Get the amount of haze present at altitude **alt_km** compared to the amount at sea level. This value is
...
### <a name="GetLocalMoonIrradianceFactor"/>vec3 GetLocalMoonIrradianceFactor(float altitude_km, simul::sky::float4 dir_to_moon, simul::sky::float4 dir_to_sun, simul::sky::HazeStruct hazeStruct)
Get the moonlight that reaches the altitude **altitude_km**.
Get the moonlight that reaches the altitude **altitude_km**.

### <a name="GetLocalSunIrradiance"/>vec3 GetLocalSunIrradiance(float altitude_km, vec3 sun_irradiance, simul::sky::float4 dir_to_sun, simul::sky::HazeStruct hazeStruct)
...
### <a name="GetPlanetRadius"/>float GetPlanetRadius()
